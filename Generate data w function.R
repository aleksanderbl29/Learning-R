  ...
  tr_df <- tr_df %>% 
    ...
    mutate(sympati = ifelse(df_id <= tr_prob[1] * length(df_id), -2,
                            ifelse(df_id <= (tr_prob[2] * length(df_id)) + (tr_prob[1] * length(df_id)), -1,
                                   ifelse(df_id <= (tr_prob[3] * length(df_id)) + (tr_prob[2] * length(df_id)) + (tr_prob[1] * length(df_id)), 0,
                                          ifelse(df_id <= (tr_prob[4] * length(df_id)) + (tr_prob[3] * length(df_id)) + (tr_prob[2] * length(df_id)) + (tr_prob[1] * length(df_id)), 1,
                                                 ifelse(df_id <= (tr_prob[5] * length(df_id)) + (tr_prob[4] * length(df_id)) + (tr_prob[3] * length(df_id)) + (tr_prob[2] * length(df_id)) + (tr_prob[1] * length(df_id)), 2, NA))))))
  return(tr_df)
}

t1_df <- assign_values(t1_df, t1_prob)
t2_df <- assign_values(t2_df, t2_prob)
t3_df <- assign_values(t3_df, t3_prob)
t4_df <- assign_values(t4_df, t4_prob)
colnames(t1_df)
